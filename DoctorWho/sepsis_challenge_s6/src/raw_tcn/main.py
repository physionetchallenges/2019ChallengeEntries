...
ex.observers.append(FileStorageObserver.create('raw_tcn_runs'))
...
@ex.main
def fit_raw_tcn(batch_size, learning_rate, training_iters, L2_penalty,  _rnd, _seed, _run, dataset, tcn_parameters):
    #Path to save model:
    ...
    #-----------------------------
    split = dataset['split']
    filepath = dataset['datapath'] + f'imputed_data_split_{split}.pkl' 
...
if __name__ == '__main__':
    ...
    plot_title = f'../../plots/raw_tcn_run_{run_id}'  
    plot_data_path = f'raw_tcn_runs/{run_id}/metrics.json' 
