...
def custom_scoring(predictions, probabilities, labels):
    # Set parameters.
    dt_early   = -12
    dt_optimal = -6
    dt_late    = 3
    ...
    num_files = len(predictions) # = number of patients! 
    ...
    #auroc, auprc        = compute_auc(pooled_labels, pooled_probabilities)
    auroc = roc_auc_score(pooled_labels, pooled_probabilities)
    auprc = average_precision_score(pooled_labels, pooled_probabilities)   
    ...
    #below code uses 'cohort' naming, rename here:
    cohort_labels = labels
    cohort_predictions = predictions 
    ...
    return auroc, auprc, normalized_observed_utility
