...
LSTM module (following mgp-rnn format)
"""
...
class LSTM_Model():
    """
    LSTM Model: LSTM predicts and returns loss
    """
    def __init__(self, tcn_params, input_dim=74, class_imb=1, L2_penalty=None):
        #n_channels, levels, kernel_size, dropout, self.n_classes = tcn_params 
        #unpack tcn parameters    
        self.n_hidden = tcn_params['n_channels']
        self.n_layers = tcn_params['levels']
        self.dropout_state = tcn_params['dropout']
        self.n_classes = tcn_params['n_classes'] 
        self.input_dim = input_dim
        self.class_imb = class_imb
        self.L2_penalty = L2_penalty
        #init LSTM:
        #lstm_cells = [tf.contrib.rnn.BasicLSTMCell(self.n_hidden) for _ in range(self.n_layers)] 
        lstm_cells = [tf.contrib.cudnn_rnn.CudnnCompatibleLSTMCell(self.n_hidden) for _ in range(self.n_layers)]
        #lstm_cells = [tf.nn.rnn_cell.GRUCell(self.n_hidden) for _ in range(self.n_layers) ]
        self.stacked_lstm = tf.contrib.rnn.MultiRNNCell(lstm_cells)
        #self.stacked_lstm = tf.nn.rnn_cell.GRUCell(self.n_hidden)
        
    def predict(self, minibatch):
        return self._get_preds(minibatch, self.stacked_lstm, 
    ...
    def _get_preds(self, minibatch, stacked_lstm, n_classes, input_dim): 
        """
        helper function. feeds raw padded data through LSTM to get predictions.
