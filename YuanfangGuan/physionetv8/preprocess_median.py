...
def feature(whole_train):
    ...
    while(i<whole_train.shape[0]):
        ...
        if (i>=7): 
            matrix_2[0:544]=whole_train[i-7:i+1,0:68].flatten()
        else:
            matrix_2[(544-(i+1)*68):544]=whole_train[0:i+1,0:68].flatten()
        ...
        if (i>=7): 
            matrix_2[544:1088]=whole_train_normed[i-7:i+1,0:68].flatten()
        else:
            matrix_2[(1088-(i+1)*68):1088]=whole_train_normed[0:i+1,0:68].flatten()

        matrix_2[1088:1156]=x_std
        matrix_2[1156:1168]=whole_train[i,68:].flatten()
    ...
    #                    string.append(0)
        ...
        matrix_4=np.ones(1248)*(-5000)
        matrix_4[0:1168][matrix_2[0:1168]!=-3000]=matrix_2[0:1168][matrix_2[0:1168]!=-3000]
        matrix_4[1168:1248]=np.sum(whole_train[:,:][whole_train[:,:]==-3000],axis=0)/(-3000.0)/float(whole_train.shape[0])

        i=i+1
    return matrix_2,matrix_4
