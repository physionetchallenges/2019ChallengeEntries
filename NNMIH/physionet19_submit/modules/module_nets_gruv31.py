...
class GRUv31(nn.Module):

    def __init__(self, args, base_list):
        super(GRUv31, self).__init__()
        ...
        self.rnn3 = ModGRU(self.h_dim, self.h_dim)
        self.lnorm1 = nn.LayerNorm(self.h_dim)
        self.lnorm2 = nn.LayerNorm(self.h_dim)
        self.lnorm3 = nn.LayerNorm(self.h_dim)
        self.lnorm4 = nn.LayerNorm(s_dim)
        self.drop1 = nn.Dropout(0.5)
        self.drop2 = nn.Dropout(0.5)
        self.drop3 = nn.Dropout(0.5)
        self.drop4 = nn.Dropout(0.5)
        self.nn1 = nn.Linear(self.h_dim, s_dim)
        self.nn2 = nn.Linear(s_dim, 2)
    ...
    def forward(self, X, M, mask):
        ...
        hx = F.relu(hx)        
        hx = self.drop2(hx)
        hx, _ = self.rnn3(hx, h0, None, mask)        
        hx = self.lnorm3(hx)
        hx = F.relu(hx)        
        hx = self.drop3(hx)
