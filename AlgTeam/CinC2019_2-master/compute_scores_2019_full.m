...
function [normalized_observed_utility] = compute_scores_2019(label_directory, testRange, prediction_directory, output_file)
...
files = files(testRange);

num_files = length(files);
% num_files = floor(length(files)/20);
...
pred_files = dir(fullfile(prediction_directory, '*.psv'));
num_files = length(pred_files);
% num_files = floor(length(files)/20);
% 
% prediction_files = cell(1, num_files);
% for k = 1 : num_files
%     prediction_files{k} = char(pred_files(k).name);
% end
% 
% 
...
%     labels        = load_column(fullfile(label_directory, label_files{k}), label_header);
    labels        = dlmread([label_directory '\' label_files{k}],'|',1,40);
%     [labels labels1]
%     predictions0   = load_column(fullfile(prediction_directory, prediction_files{k}), prediction_header);
%     probabilities0 = load_column(fullfile(prediction_directory, prediction_files{k}), probability_header);
    tmp        = dlmread([prediction_directory '\' label_files{k}],'|',1,0);
    predictions = tmp(:,2);
    probabilities = tmp(:,1);
%     [predictions0 predictions]
%     [probabilities0 probabilities]
