...
os.system('rm -rf utility_figure_data')
os.system('mkdir utility_figure_data')
...
for the_file in result_all:
    ...
    NEW=open(('utility_figure_data/'+nametable[-1]),'w')
    i=0
    while (i<len(gs)):
        print(pred[i],scoring_cost[i])
        val=(pred[i]-0.5)*scoring_cost[i]
