...
class GetStatistic(FeatureSaverMixin):
    ...
    def __init__(self, statistic='std', columns=None, lookback=7, last_only=False):
        self.columns = columns
        self.lookback = lookback
        ...
        # Save location for Mixin
        self.save_loc = DATA_DIR + '/processed/features/basic_statistics/stat={}_lookback={}_columns={}.pickle'.format(statistic, lookback, str(columns)[0:100])

    def func(self, df):
        if self.statistic == 'min':
            return df.rolling(self.lookback, min_periods=2).min()
        elif self.statistic == 'max':
            return df.rolling(self.lookback, min_periods=2).max()
        elif self.statistic == 'std':
            return df.rolling(self.lookback, min_periods=2).std()

    def transform_func(self, df):
        if self.last_only is False:
            return df.groupby('id').apply(self.func)
        else:
            df = df.iloc[-8:].groupby('id').apply(self.func)
            return df.iloc[[-1]].values
            # if self.statistic == 'min':
            #     return np.nanmin(vals[-self.lookback:, :], axis=0).reshape(1, -1)
            # elif self.statistic == 'max':
            #     return np.nanmax(vals[-self.lookback:, :], axis=0).reshape(1, -1)
            # elif self.statistic == 'std':
            #     return vals[-self.lookback:, :].std(axis=0).reshape(1, -1)
...
if __name__ == '__main__':
    ...
    moments = SignaturesFromIdDataframe(columns=['SOFA'], options={'leadlag': True}, last_only=True).transform(df)
